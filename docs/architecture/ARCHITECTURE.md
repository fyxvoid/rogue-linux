...
## 4. The Advisor (`cogman-src-advisor`)
A specialized **Rust** crate for AI interactions.
-   **Responsibility**: Interface with the Neural Engine (Python/LLama.cpp).
-   **Integration**: Used by the Planner to provide "British Butler" advice during build failures.

## Directory Structure
-   `cogman/src/`: The Modular Workspace.
    -   `planner/`: Rust build system logic.
    -   `executor/`: C execution engine.
    -   `advisor/`: AI interface crate.
-   `cogman/bin/`: Production binaries (`cogman-planner`, `cogman-exec`).
-   `docs/`: Comprehensive system documentation.
-   `training/`: AI model training pipeline.
-   `website/`: Custom SSG-based frontend.
