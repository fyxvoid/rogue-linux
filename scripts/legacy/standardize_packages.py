...
def standardize_pkg(pkg_path, pkg_name, cat_name):
    ...
    butler.log_check(f"Standardizing configuration for {cat_name}/{pkg_name}")
    ...
    if os.path.exists(ROOT_TAR_DIR):
        for f in os.listdir(ROOT_TAR_DIR):
            if f.startswith(pkg_name) and (f.endswith(".tar.gz") or f.endswith(".tar.xz") or f.endswith(".tar.bz2")):
                ...
                butler.log_info(f"Relocating {f} to local package tar directory")
...
def main():
    if not os.path.exists(PACKAGES_DIR):
        butler.log_error(f"Packages directory not found: {PACKAGES_DIR}")
        return
    ...
    butler.log_success("Package standardization process complete")
